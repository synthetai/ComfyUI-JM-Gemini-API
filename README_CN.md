...
### 水印去除
- 自动检测并去除Gemini生成图像上的水印
- 支持48×48和96×96两种水印尺寸
- 使用反向alpha混合算法实现干净去除
- 简单的一键操作 - 只需将图像传递给节点即可
...
### 节点：JM Gemini Watermark Remover（JM Gemini水印去除器）

该节点用于去除Gemini生成图像上显示的水印。

#### 输入：
- **image**：带有水印的Gemini生成图像（IMAGE tensor格式）

#### 输出：
- **image**：去除水印后的图像（IMAGE tensor格式）

#### 使用方法：

1. 在工作流中添加"JM Gemini Watermark Remover"节点
2. 将"JM Gemini Image Generator"的输出（或任何包含Gemini图像的Load Image节点）连接到image输入
3. 将输出连接到Preview Image或Save Image节点
4. 运行工作流

节点将会：
- 根据图像尺寸自动检测水印大小（48×48或96×96）
- 计算水印位置（右下角）
- 使用反向alpha混合算法去除水印
- 将处理后的图像保存到ComfyUI的output目录

**工作原理：**
- Gemini会为大于512×512像素的图像添加水印
- 大于1024×1024的图像使用96×96水印
- 较小的图像使用48×48水印
- 节点使用从背景捕获预计算的alpha映射来反向混合水印

## 更新日志

### 版本 1.2.0
- 新增JM Gemini Watermark Remover水印去除节点
- 自动去除Gemini生成图像上的水印
- 支持48×48和96×96两种水印尺寸
- 使用反向alpha混合算法实现干净去除
